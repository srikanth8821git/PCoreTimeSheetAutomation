...
exports.TimesheetPage =
  class TimesheetPage {
    ...
    async addRemarksToHoursFilled(projectremarks) {
      ...
      for (let i = 0; i < await mainFrame.locator(this.addremarks).count(); i++) {
        if (await mainFrame.locator(this.addremarks).nth(i).isVisible()) {
          ...
          await mainFrame.locator(this.closeremarkswindow).nth(i).click();
          await mainFrame.locator(this.addremarks).nth(i).click();
          await expect(mainFrame.locator(this.remarkstext).nth(i)).toHaveValue(projectremarks);
          await mainFrame.locator(this.closeremarkswindow).nth(i).click();
        }
      }
    }
    ...
    async addProjectDetails(projectgroup, projectactivity) {
      ...
      await mainFrame.locator(this.selectprojectgroup).nth(2).selectOption(projectgroup);
      await mainFrame.locator(this.selectprojectactivity).nth(2).selectOption(projectactivity);
    ...
    async verifyTimeSheetDates(monday, sunday, previousDate) {
      ...
      await expect(await mainFrame.locator(this.timesheetLastSubmittedLabel(msg))).toBeVisible();
    ...
    async addTimesheetHours() {
      ...
      for (let i = 14; i < 19; i++) {
        ...
        if (await this.page.frameLocator(this.frameMain).locator(this.selectHours).nth(i).isEnabled()) {
          await this.page.frameLocator(this.frameMain).locator(this.selectHours).nth(i).selectOption('8');
        }
      }
      ...
      for (let i = 0; i < 21; i++) {
        ...
        totalUserEnteredHours = totalUserEnteredHours + parseFloat(await this.page.frameLocator(this.frameMain).locator(this.selectHours).nth(i).inputValue());
      }
    ...
    async submitTimeSheet(totalUserEnteredHours) {
      ...
      const totalHours = await this.page.frameLocator(this.frameMain).locator(this.totalhourscalculated).nth(0).textContent();

      if (toString(totalHours) == toString(totalUserEnteredHours) && totalUserEnteredHours == 40) {
